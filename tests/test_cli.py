...
def test_deposit_lightning_create_calls_endpoint(runner, config_store):
    ...
    responses.add(
        responses.POST,
        f"{DEFAULT_API_BASE_URL}/wallet/deposit-lightning",
...
def test_tx_list_returns_transactions(runner, config_store):
    ...
    responses.add(
        responses.GET,
        f"{DEFAULT_API_BASE_URL}/transactions/my-transactions",
...
@responses.activate
def test_tx_direct_payment_create_forwards_optional_fields(runner, config_store):
    config_store.save(ConfigData(api_base_url=DEFAULT_API_BASE_URL, auth_type="api_key", api_key="key-123"))
    responses.add(
        responses.POST,
        f"{DEFAULT_API_BASE_URL}/transactions/direct-fiat/tentatives",
...
def test_contacts_favourite_uses_form_data(runner, config_store):
    ...
    responses.add(
        responses.POST,
        f"{DEFAULT_API_BASE_URL}/contacts/is-favourite",
...
def test_tx_inner_transfer_uses_form_data(runner, config_store):
    ...
    responses.add(
        responses.POST,
        f"{DEFAULT_API_BASE_URL}/transactions/inner-transfer",
...
def test_user_spending_limit_calls_endpoint(runner, config_store):
    ...
    responses.add(
        responses.GET,
        f"{DEFAULT_API_BASE_URL}/users/spending-limit",
...
def test_user_settings_get_calls_endpoint(runner, config_store):
    ...
    responses.add(
        responses.GET,
        f"{DEFAULT_API_BASE_URL}/users/user-settings",
...
@responses.activate
def test_user_b2b_token_create_mints_token(runner, config_store):
    config_store.save(ConfigData(api_base_url=DEFAULT_API_BASE_URL, auth_type="api_key", api_key="key-123"))
    responses.add(
        responses.POST,
        f"{DEFAULT_API_BASE_URL}/users/b2b/sub-1/api-token",
        json={"token": "sk-sub-1", "wapu_user_id": "sub-1"},
        status=201,
    )

    result = runner.invoke(cli, ["--output", "json", "user", "b2b", "token", "create", "sub-1"])
...
@responses.activate
def test_user_b2b_token_revoke_uses_delete(runner, config_store):
    config_store.save(ConfigData(api_base_url=DEFAULT_API_BASE_URL, auth_type="api_key", api_key="key-123"))
    responses.add(
        responses.DELETE,
        f"{DEFAULT_API_BASE_URL}/users/b2b/sub-1/api-token",
    ...
    result = runner.invoke(cli, ["--output", "json", "user", "b2b", "token", "revoke", "sub-1"])
...
def test_user_b2b_token_create_requires_authentication(runner):
    result = runner.invoke(cli, ["user", "b2b", "token", "create", "sub-1"])
